 ...
 *   <li><code>isUDPPortInUse(int)</code> - Checks if a specified UDP port is in use.</li>
 * </ul>
 */
public class UDPPackage {
    ...
    public static void send(JFrame mainFrame, String printerIP, String PrinterSN, String PrinterModel, String PrinterName) {
        ...
        int remoteUdpPort = 2021;              // UDP port for Bambu Studio
        int sourceUdpPort = 0;                 // Source port (0 = system assigned)

        // Check if the target UDP port is in use
        boolean isPortInUse = isUDPPortInUse(remoteUdpPort);

        if (isPortInUse) {
            try {
                // Current date for the HTTP headers
                ...
                // Show a warning that package could not send to Bambu Studio
                new DialogOneButton(mainFrame, null, new ImageIcon(MainMenu.class.getResource("/achtung.png")), "<html>Warning! Failed to send the package to Bambu Studio!</html>", "Ok").showDialog();
            }
        } else {
    ...
    /**
     * Checks if a specified UDP port is in use.
     *
